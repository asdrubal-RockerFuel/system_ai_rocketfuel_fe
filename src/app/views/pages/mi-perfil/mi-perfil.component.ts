...
@Component({
  ...
  standalone: true,
  imports: [clientService, companyService]
})
export class MiPerfilComponent implements OnInit {
  ...
  getTab2() {
    ...
    persona.forEach((img: any) => {
      this.personasPreview.push(img.value)
    })

    persona.forEach((item: any) => {
      const mediaControl = this.formBuilder.group({
        archivo: '',
        change: false,
        recovered: [item.value],
  ...
  arrFilesToShow = [];
  arrFilesDataToSend: ElementoImg[] = [];
  arrFilesToSend: File[] = [];
  ...
  sendTabForm(formName: any, body: any) {
    this.loadingTab = true;
    if (formName.valid) {
