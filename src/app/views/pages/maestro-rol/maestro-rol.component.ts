...
@Component({
  selector: "app-maestro-rol",
  templateUrl: "./maestro-rol.component.html",
  styleUrls: ["./maestro-rol.component.scss"],
})
export class MaestroRolComponent implements OnInit {
    ...
    this.modalService
      ...
      .result.then(
        (result) => {
          ...
          this._sRol.disableRole(body, row.id ).subscribe(
            (resp) => {
              this.toastr.success(
                "Cambio realizado",
  ...
  entraPermisos(item) {
    this.router.navigate([`/pages/maestro-permisos/${item.id}`]);
  }
