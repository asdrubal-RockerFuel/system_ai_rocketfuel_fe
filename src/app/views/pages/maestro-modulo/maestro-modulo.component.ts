...
@Component({
  selector: "app-maestro-modulo",
  templateUrl: "./maestro-modulo.component.html",
  styleUrls: ["./maestro-modulo.component.scss"],
})
export class MaestroModuloComponent implements OnInit {
  constructor(
  ...
  validarContratacion() {
    ...
    if (
      this.myForm.get("area_name").value == "" ||
      this.myForm.get("area_name").value == null ||
      this.myForm.get("area_name").value == undefined
    ) {
  ...
  editarArticuloAdmin() {
    ...
    if (this.DatosUpdate !== JSON.stringify(this.DatosArea)) {
      (de.id = this.DatosArea.id),
        ...
        (de.area_name = this.DatosArea.area_name),
        (this.workback = true);
      console.log("de", de);
